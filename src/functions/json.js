...
class JsonDatabase {
    constructor({ databasePath = "databases/db.json", maxDataSize = null } = {}) {
        if (maxDataSize !== null && typeof maxDataSize !== "number") {
            console.log("DATABASE | The maximum limit must be in number type!");
        }

        if (maxDataSize !== null && maxDataSize < 1) {
            console.log("DATABASE | Inappropriate range for the limit!");
        }
    ...
    set(key, value, autoWrite = true) {
        if (key === "" || typeof key !== "string") {
            console.log("DATABASE | Unapproved key!");
        }

        if (value === "" || value === undefined || value === null) {
            console.log("DATABASE | Unapproved value!");
        }

        if (typeof autoWrite !== "boolean") {
            console.log("DATABASE | autoWrite parameter must be true or false!");
        }

        if (typeof this.maxDataSize === "number" && this.size >= this.maxDataSize) {
            console.log("DATABASE | Data limit exceeded!");
        }
    ...
    all(limit = 0) {
        if (typeof limit !== "number") {
            console.log("DATABASE | Must be of limit number type!");
        }
    ...
    pull(key, callbackfn, multiple = false, thisArg) {
        ...
        if (!Array.isArray(data)) console.log(`DATABASE | ${key} It is not a data string with an ID.`);
        if (typeof multiple !== "boolean") {
            console.log("DATABASE | multiple parameter must be true or false!");
        }
    ...
    math(key, operator, value, goToNegative = false) {
        if (Array.isArray(value) || isNaN(value)) {
            console.log(`DATABASE | The type of value is not a number.`);
        }

        if (value <= 0) console.log(`DATABASE | Value cannot be less than 1.`);
        value = Number(value);
        if (typeof goToNegative !== "boolean") console.log(`DATABASE | The goToNegative parameter must be of boolean type.`);
        let data = this.get(key);
        if (!data) {
            return this.set(key, value);
        }
        if (Array.isArray(data) || isNaN(data)) console.log(`DATABASE | ${key} ID data is not a number type data.`);
